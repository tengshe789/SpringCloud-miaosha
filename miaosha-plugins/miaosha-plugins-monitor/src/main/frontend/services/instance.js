...
class Instance {
    ...
    hasEndpoint(endpointId) {
        return this.endpoints.findIndex(endpoint = > endpoint.id === endpointId
    ) >=
        0;
    }
    ...
    fetchHealth() {
        return await
        this.axios.get(uri`actuator/health`, {
            headers: {'Accept': actuatorMimeTypes}
        });
    }
    ...
    hasEnvManagerSupport() {
        ...
        this.axios.options(uri`actuator/env`);
    ...
    fetchAuditevents({after, type, principal}) {
    ...
    fetchSessionsByUsername(username) {
    ...
    readMBeanAttributes(domain, mBean) {
        const body = {
            type: 'read',
            mbean: `${domain}:${mBean}`,
    ...
    writeMBeanAttribute(domain, mBean, attribute, value) {
        const body = {
            type: 'write',
            mbean: `${domain}:${mBean}`,
            attribute,
    ...
    invokeMBeanOperation(domain, mBean, operation, args) {
        const body = {
            type: 'exec',
            mbean: `${domain}:${mBean}`,
            operation,
